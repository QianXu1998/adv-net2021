...
- [Utilities (./cli.py)](#utilities-clipy)
    ...
    - [`./cli.py flow-performance [path]`](#clipy-flow-performance-path)
...
The project will start on week 7; at this point, you will get access to new VMs for the project, and will be able to start programming your network.
...
As a general rule, we add `2.5ms` of delay for each 250km uo tp 2000km. Above 2000km, we set a fix delay of `25ms`.
...
> :rotating_light: Keep in mind that your code will be reviewed. Cheating using controller code is easy, but it's unlikely that we won't catch it when looking at your code... Let it be said: cheaters will be severely punished.
...
In each run of the simulation, we collect various information (delay, packet reception, etc.) required to assess whether the various SLA are met. The simulation directly outputs which SLAs your network satisfies.

The SLA to meet are specified TODO: How? Where? I guess a csv file in the infra repo.

TODO: fill-in the first SLA to meet (basic things about packet reception)
...
The final competition will consider a set of scenarios (exact number to be confirmed) using different traffic and failure inputs randomly selected from the [input pool](https://gitlab.ethz.ch/nsg/public/adv-net-2021-project-inputpool). Points added over all scenarios
